...
gqlRequest
  :: forall row . ArgonautCodecs.DecodeJson (Record row)
  => Affjax.URL
  -> (SelectionSet row RootQuery)
  -> Aff (Either GraphqlError { data :: Record row })
gqlRequest url selectionSet = Transformers.runExceptT do
  result <- Transformers.lift $ Affjax.post
  ...
  case result of
    Left error ->
      Transformers.throwError $ AffjaxError error
    Right response ->
      let
          (json :: ArgonautCore.Json) = response.body
       in case ArgonautCodecs.decodeJson json of
