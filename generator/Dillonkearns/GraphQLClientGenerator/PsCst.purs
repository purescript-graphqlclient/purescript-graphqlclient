module Dillonkearns.GraphQLClientGenerator.PsCst where
...
import Dodo as Dodo
import Dillonkearns.GraphQLClient.Utils (anyPredicate)
import Dillonkearns.GraphQLClientGenerator.IntrospectionSchema (InstorpectionQueryResult, InstorpectionQueryResult__Field, InstorpectionQueryResult__FullType)
import Dillonkearns.GraphQLClientGenerator.IntrospectionSchema.TypeKind as TypeKind
import Dillonkearns.GraphQLClientGenerator.MakeModule.Enum as MakeModule.Enum
import Dillonkearns.GraphQLClientGenerator.MakeModule.InputObject as MakeModule.InputObject
import Dillonkearns.GraphQLClientGenerator.MakeModule.Interface as MakeModule.Interface
import Dillonkearns.GraphQLClientGenerator.MakeModule.Mutation as MakeModule.Mutation
import Dillonkearns.GraphQLClientGenerator.MakeModule.Object as MakeModule.Object
import Dillonkearns.GraphQLClientGenerator.MakeModule.Query as MakeModule.Query
import Dillonkearns.GraphQLClientGenerator.MakeModule.Scalars as MakeModule.Scalars
import Dillonkearns.GraphQLClientGenerator.MakeModule.Scopes as MakeModule.Scopes
import Dillonkearns.GraphQLClientGenerator.MakeModule.Subscription as MakeModule.Subscription
import Dillonkearns.GraphQLClientGenerator.MakeModule.Union as MakeModule.Union
...
fullTypeToModuleMapItem mkModule apiModuleName submodule fullType =
  ...
  in
    Tuple name (printModuleToString $ mkModule moduleName fullType)
...
mkFilesMap customScalarsModule apiModuleName introspectionQueryResult =
  let
    ...
    nameToScope = doQuery >>> doMutation >>> doSubscription >>> StringsExtra.pascalCase >>> (\x -> "Scope__" <> x)
      ...
      doQuery str =
        if str == introspectionQueryResult.__schema.queryType.name then
          "RootQuery"
        else
          str

      doMutation :: String -> String
      doMutation str = case introspectionQueryResult.__schema.mutationType of
        Nothing -> str
        Just { name } ->
          if str == name then
            "RootMutation"
      ...
      doSubscription str = case introspectionQueryResult.__schema.subscriptionType of
        Nothing -> str
        Just { name } ->
          if str == name then
            "RootSubscription"
  ...
  in
    ...
    , files:
      { "InputObject":
        ...
        in
          printModuleToString $ MakeModule.InputObject.makeModule scalarModule apiModuleName moduleName inputObjectTypes
      , "Query":
        let
            ...
            let
              ...
              scalarTypes =
                introspectionQueryResult.__schema.types
                  ...
                  # Array.filter (\fullType -> not $ elem fullType.name builtInScalarNames)
            in
              Just $ printModuleToString $ MakeModule.Scalars.makeModule moduleName scalarTypes
      , "Mutation":
        do
          ...
          (fields :: Array InstorpectionQueryResult__Field) <-
            mutation.fields
              >>= case _ of
                  [] -> Nothing
                  arr -> Just arr
          let
            moduleName = mkModuleName $ apiModuleName <> NonEmpty.singleton "Mutation"
          ...
          (fields :: Array InstorpectionQueryResult__Field) <-
            subscription.fields
              >>= case _ of
                  ...
                  arr -> Just arr
          let
            moduleName = mkModuleName $ apiModuleName <> NonEmpty.singleton "Subscription"
          Just $ printModuleToString
