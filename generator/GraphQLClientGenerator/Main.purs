...
-- generating query with custom generator
-- this generator doesn't add aliases (i.e. `alias123` in query `query { alias123: search(a: 1) { fooField }}`)
-- by default we add aliases everywhere
-- by here we need to generate query string that is equal to the one generated by graphql npm library
